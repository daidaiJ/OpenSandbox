...
- **触发条件修正**：病理与「容量墙」无关——在无容量墙（50m 超轻模板、远低于节点余量）条件下同样复现；充分条件是「慢启动（未 Ready 在途滞留于误计的 buffer 口径内）+ 突发负载」，**风暴循环不依赖请求失败**（测试环境 readiness 70s > 60s 总闸的失败潮只是加速器；生产创建超时配 240s 同样中招）。定量门槛 `alloc > 2×supply + 3×midpoint`（推导见 [触发条件证据链](../wiki/opensandbox-pool-scalein-trigger-evidence-chain.md)，同时解释小池不可触发与 <1/3 水位触发）。上游 #1423 的 1355 Pending 超限是 terminating 计数缺陷的症状而非根因。
